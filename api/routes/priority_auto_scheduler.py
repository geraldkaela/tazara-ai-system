...
async def create_auto_schedule(
    ...
    try:
        ...
        # Define actual train capacities (same as frontend)
        train_capacities = {
            1: 500,   # Heavy Freight
            2: 350,   # Medium Freight
            3: 350,   # Medium Freight
            4: 250,   # Light Freight
            5: 250,   # Light Freight
            6: 600    # Super Heavy
        }
        ...
        for route_key, route_orders in route_groups.items():
            if route_orders and day < request.max_days:
                ...
                # Check if multiple trains are justified
                multi_train_justified = (avg_priority >= 75 and route_cargo > 600)
                max_trains_for_route = 2 if multi_train_justified else 1
                ...
                while remaining_route_cargo > 0 and trains_assigned < max_trains_for_route and train_index < len(sorted_train_ids):
                    ...
                    train_id = sorted_train_ids[train_index % len(sorted_train_ids)]
                    train_capacity = train_capacities[train_id]
                    
                    # Calculate cargo for this train - use actual route cargo, not train capacity
                    cargo_to_assign = min(remaining_route_cargo, route_cargo)
        ...
        actual_days_used = len(daily_assignments)
